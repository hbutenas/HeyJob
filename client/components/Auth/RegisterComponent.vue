<template>
  <AuthComponent @authEmit="handleSubmitButton">
    ...
    <div class="mb-4">
      ...
      <input class="border rounded w-full py-2 px-3 mt-1 text-gray-700 leading-tight focus:outline-none" id="email"
    ...
    <div class="mb-4">
      ...
      <input class="border rounded w-full py-2 px-3 mt-1 text-gray-700 leading-tight focus:outline-none" id="password"
        type="password" placeholder="******************" v-model="password" required>
    </div>
...
const handleSubmitButton = async () => {
  ...
  const errors = response.error.value && response.error.value.data ? response.error.value.data.errors : false;

  // error occured while creating new users  
  if (errors) {
